...
| 针对文件所有者         | 针对与文件所有者  同组的用户 | 针对非同组的用户         |
| ---------------------- | ---------------------------- | ------------------------ |
| 指定所有者是否可读     | 指定组用户是否可读           | 指定其他用户  是否可读   |
| 指定所有者是否可写     | 指定组用户是否可写           | 指定其他用户  是否可写   |
| 指定所有者  是否可执行 | 指定组用户  是否可执行       | 指定其他用户  是否可执行 |

struct stat st_mode; 包含了文件访问权限位

st_mode低11bit

st_mode[6]：指示文件所以者是否可执行

st_mode[7]：指示文件所以者是否可写

st_mode[8]：指示文件所以者是否可读
