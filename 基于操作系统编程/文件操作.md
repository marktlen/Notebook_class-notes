...
![两个独立进程各自打开同一个文件](文件操作.assets/两个独立进程各自打开同一个文件.png)
...
![不同进程共享文件对象](文件操作.assets/不同进程共享文件对象.png)
...
![同一个进程打开不同的文件](文件操作.assets/同一个进程打开不同的文件.png)

### 同一个进程内的共享（dup等）

![进程内共享文件](文件操作.assets/进程内共享文件.png)
...
![同一进程内线程共享](文件操作.assets/同一进程内线程共享.png)
...
![image-20200324111834048](文件操作.assets/image-20200324111834048.png)
...
- 如果试图设置普通文件的粘住位（S_ISVTX），而且又没有超级用户权限，则mode中的粘住位自动被关闭。这意味着只有超级用户才能设置普通文件的粘住位。
  ...
  - | 常数    | 说明         | 对普通文件的影响                   | 对目录的影响                                 |
    ...
    | S_ISGID | 设置-组-ID   | 若组执行位设置，则执行时置有效组ID | 将在目录中创建新文件的  组ID设置为目录的组ID |
